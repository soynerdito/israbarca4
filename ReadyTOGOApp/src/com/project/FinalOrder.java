...
//        helpButton.setOnClickListener((OnClickListener) new helpListener());
		...
		for(j=0;j<i;j++)
		{
			...
			if(cat==1)
			{
				httpget = new HttpGet("http://127.0.0.1:8000/appetizers.json");
			}
			else if(cat==2)
			{
			...
			{
				httpget = new HttpGet("http://127.0.0.1:8000/lab/salad.json");
			}
			else if(cat==6)
			{
				httpget = new HttpGet("http://127.0.0.1:8000/lab/desserts.json");
			}
			...
			try {
				...
				for(int l=1;l<arr.length();l++)
				{
					...
					if(name.equals(name1))
					{
						...
						sr1.setamount("$. " + Integer.toString(amount));
						items.add(sr1);
						break;
					}
				}
		...
		httpget1 = new HttpGet("http://127.0.0.1:8000/rates.json");
		...
		total = total + tax;
		disText.setText("Discount (@"+dis+"%) = $. "+Double.toString(dis1));
		taxText.setText("Tax (@11.5%) = $. " + Double.toString(tax));
